...
movies = [movie1]
@app.post(path='/login', tags=['authentication'])
def login(user: User):
...
@app.get(path= '/', tags=['Inicio'], dependencies=[Depends(BearerJWT())])
def read_root():
    return HTMLResponse('<h1>Hello Movies</h1>')

@app.get(path= '/movies', tags=['Movies'], dependencies=[Depends(BearerJWT())])
def get_movies():
    return movies
